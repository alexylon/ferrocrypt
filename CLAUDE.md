...
| `archive/decode.rs` | TAR extraction. Owns `unarchive`, both `extract_entries` arms (Linux/macOS hardened path + path-based fallback) which iterate via `archive.entries()?.raw(true)` so PAX/GNU/Solaris extension records surface as their own entries instead of being silently merged into the next entry by the `tar` crate, `validate_ustar_entry` (typeflag match rejects every PAX `'x'`/`'g'` and GNU `'L'`/`'K'`/`'S'`/`'M'`/`'D'`/`'V'`/`'N'` and Solaris `'X'` record explicitly), `NormalizedEntry`, `ExtractCounters`, `drain_and_verify_zero_padding`, `extract_and_register_root`, `incomplete_working_name`, `incomplete_entry_path` (fallback only), and `restore_permissions_from_mode` (Unix non-Linux/non-macOS + non-Unix). Decryption writes under an `.incomplete` working name throughout and renames to the final name on success; failure leaves the `.incomplete` on disk for inspection. |
