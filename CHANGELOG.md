...
- **Fuzz-target rewrite around parser surfaces, plus CI wiring.** Eight parser-surface targets exercise the lowest-useful parsing layers directly, skipping the Argon2id derivation that dominates the integration targets: `fuzz_header_prefix` (27-byte triple-replicated prefix + magic/type + canonicity), `fuzz_private_key_header` (v1 `private.key` header + structural shape check), `fuzz_kdf_params` (12-byte KDF parameter bounds), `fuzz_tlv` (canonical TLV grammar + reserved/critical tag rejection), `fuzz_replication_decode` (majority-vote decoder), `fuzz_archive_path` (tar-entry path guard), `fuzz_recipient_decode` (Bech32 `fcr1…`), and `fuzz_detect_mode` (top-level encryption-mode detection). Two integration targets (`fuzz_symmetric_decrypt`, `fuzz_hybrid_decrypt`) cover the end-to-end decrypt pipeline. A new `fuzz` job in GitHub Actions installs the nightly toolchain and `cargo-fuzz`, runs clippy on the fuzz crate, builds every target on every push/PR (catching API drift), smoke-fuzzes every parser-surface target for ten seconds, and uploads crash reproducers as a workflow artifact on failure. Internals are re-exported through a `ferrocrypt/fuzzing` cargo feature that is off by default and not part of the stable API.
...
- **Breaking:** New on-disk **format v1** across every artefact (single `.fcr` extension replacing `.fcs` / `.fch`). 4-byte ASCII magic `"FCR\0"`. Single `version` byte per file. Symmetric and hybrid share an 8-byte prefix layout (`magic(4) | version(1) | type(1) | ext_len(2)`) distinguished only by the `type` byte (`'S'` / `'H'`). See `ferrocrypt-lib/FORMAT.md` for the full spec.
- **Breaking:** Both modes use a per-file **32-byte `file_key`** indirection (age-inspired): the passphrase (symmetric) or X25519 ECDH (hybrid) produces a `wrap_key` via HKDF-SHA3-256, which seals the random `file_key` via XChaCha20-Poly1305. The payload AEAD key and the header HMAC key are then derived from `file_key` via HKDF-SHA3-256. Payload encryption is XChaCha20-Poly1305 STREAM-BE32 over 64 KiB chunks; HMAC-SHA3-256 authenticates the on-disk replicated prefix + envelope + stream nonce + extension region, and is verified **after** the envelope unwraps the `file_key` — so wrong-passphrase / wrong-key fails at envelope unwrap and header tamper fails at HMAC verification as distinct error variants.
- **Breaking:** Triple replication is **narrowed to the 8-byte prefix only**. Body fields (salts, nonces, envelope, ext_bytes, HMAC tag, ciphertext) are stored raw — they are either random or cryptographically authenticated, so per-byte rot cannot be silently corrected there. The replicated prefix is now enforced canonically: writers emit three byte-identical copies, and readers majority-decode + re-encode + compare against on-disk bytes, rejecting `CorruptedPrefix { decoded_view }` on mismatch. The decoded view carries the majority-voted version / type so "Newer file format, upgrade FerroCrypt" diagnostics still surface on a bit-rotten file. Header size is **194 bytes** (symmetric) / **182 bytes** (hybrid).
- **Breaking:** `public.key` is now a **UTF-8 text file** containing the canonical Bech32 `fcr1…` recipient string (+ optional trailing LF). No binary format; no separate integrity tag (BIP 173 checksum provides copy-paste robustness). The Bech32 payload is `algorithm(1) || public_key_material(32)` = 33 bytes; for X25519 the file is ~64 bytes on disk.
- **Breaking:** `private.key` v1 is passphrase-protected via **Argon2id + HKDF-SHA3-256 + XChaCha20-Poly1305** and written with `0o600` on POSIX. Layout (125 bytes when `ext_len = 0`): cleartext header (9 B: magic + version + type `0x4B 'K'` + algorithm + ext_len) → argon2_salt(32) → kdf_params(12) → wrap_nonce(24) → ext_bytes → wrapped_privkey(48). Binds every cleartext byte before `wrapped_privkey` as **AEAD associated data** so tampering any field fails authentication. The AEAD primitive returns a single undifferentiated failure for both wrong-passphrase and tampered-cleartext — both surface as the same `KeyFileUnlockFailed` error, whose Display wording (`"Private key unlock failed: wrong passphrase or tampered file"`) reflects both causes. The type byte `0x4B` is deliberately distinct from the symmetric `.fcr` type `0x53` so `file(1)`-style matchers can disambiguate.
- **Breaking:** Forward-compatible authenticated **TLV extension region** (`ext_bytes`, empty today with `ext_len = 0`) is present in both `.fcr` and `private.key`. Entries are `tag(u16 BE) || len(u16 BE) || value`. Tags in `0x0001..=0x7FFF` are ignorable (authenticate + skip); `0x8001..=0xFFFF` are critical (reject with `UnknownCriticalTag`); `0x0000` / `0x8000` are reserved. `ext_len` is capped at 32 KiB. Rules are normative in FORMAT.md §6.
- **Breaking:** SHA-3 unified across the spec — HMAC-SHA3-256 (header), HKDF-SHA3-256 (every derivation: symmetric wrap, hybrid wrap, private-key wrap, payload subkey, header subkey), SHA3-256 fingerprint.
- **Breaking:** Public-key fingerprints hash `algorithm || key_material` (33 bytes for X25519) via SHA3-256. Cross-algorithm safety: future public-key algorithms cannot collide with X25519 fingerprints.
- **Breaking:** Symmetric password-based key derivation pipeline: **Argon2id** (1 GiB memory / time cost 4 / parallelism 4) → **HKDF-SHA3-256** → `wrap_key`. The wrap key seals the random 32-byte `file_key`, and `payload_key` + `header_key` are subsequently derived from `file_key`. A compromise of one subkey does not reveal the other. KDF parameters are stored in the symmetric envelope so decryption always uses the exact values from encryption time.
...
- **Decryption errors are now distinct and user-readable.** Previously a failed decryption surfaced as a wrapped internal error such as `Io(Custom { ..., error: "aead::Error" })` on the CLI, with no way for a user to tell whether the password was wrong, the header was tampered with, the payload was corrupted mid-stream, or the file was simply truncated. Seven distinct error conditions are now reported, each with a short plain-language message:
    ...
    - **Decryption failed: wrong passphrase or tampered envelope** — (symmetric) the passphrase does not unlock the envelope, or the envelope was tampered with.
    - **Decryption failed: wrong private key or tampered envelope** — (hybrid) the supplied private key does not match the envelope's recipient, or the envelope was tampered with.
    - **Decryption failed: header tampered after unlock** — the envelope unwrapped successfully but the HMAC over the rest of the header failed. The right key is present, but the file's `stream_nonce` or `ext_bytes` were modified after signing. Distinct from envelope failure so users know it's a tamper event, not a credential mistake.
    ...
    - **Encrypted file is truncated** — the encrypted stream ends before its final-flag authenticated chunk.
    - **Encrypted file has unexpected trailing data** — bytes remain after the final-flag chunk. STREAM-BE32's per-chunk nonce binding rejects ordinary appends earlier as a payload-authentication failure (above); this dedicated variant fires from the post-`decrypt_last` trailing-data probe in `DecryptReader::fill_buffer` for pathological readers that signal end-of-file at the chunk boundary and then yield more bytes.
...
- **Pub/priv key file mix-ups are now reported as `WrongKeyFileType`** across every entry point. Reading a binary `private.key` through `read_public_key` used to surface a cryptic `io::Error` "stream did not contain valid UTF-8"; reading a text `public.key` through `read_private_key` or `validate_private_key_file` used to surface the generic `NotAKeyFile` defect. Both paths now detect the swap (with leading-whitespace tolerance on the `fcr1…` sniff) and return `FormatDefect::WrongKeyFileType`.
- **`public.key` tolerates surrounding whitespace.** Leading / trailing ASCII whitespace (spaces, tabs, CR, LF) is stripped before Bech32 decode, so files saved with a missing trailing newline, Windows CRLF line endings, or an editor-inserted trailing blank line all parse. Internal whitespace is still rejected by the Bech32 charset.
- **Missing key-file paths surface as `InputPath`.** `read_public_key`, `read_private_key`, and `validate_private_key_file` now map `io::ErrorKind::NotFound` to `CryptoError::InputPath`, matching the upfront `validate_input_path` behaviour instead of leaking a raw OS error string.
