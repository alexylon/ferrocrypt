...
- **Independent version domains for `.fcr`, `private.key`, and `public.key`.** FerroCrypt now exposes a single `KeypairSuite` enum (today: `V1`) that both key parsers translate their on-disk encoding into before deciding support, and three independent version constants (`FCR_FILE_VERSION`, `PRIVATE_KEY_VERSION`, `PUBLIC_KEY_VERSION`) that no longer share a backing byte. A bump to the outer `.fcr` file format does not bump key compatibility, and a key-pair suite bump does not bump the file format. The encrypt/decrypt symmetry rule (`FORMAT.md` §11) — "a release MUST NOT accept a public key for encryption unless the same key-pair suite remains supported for private-key decryption" — is now structurally enforced through one shared support gate, `keypair_suite_is_supported`. New `CryptoError::UnsupportedVersion` variants `OlderPublicKey { version }` and `NewerPublicKey { version }` carry the rejected public-recipient version separately from the existing `OlderKey` / `NewerKey` variants for `private.key` rejection. The new `PublicKeyWireVersion` enum is exposed for callers that want to introspect a recipient payload's wire shape (legacy unversioned vs future versioned).
...
- **Breaking:** New on-disk **format v2** across every artefact. Single `.fcr` extension replaces `.fcs` / `.fch`. v2 `.fcr` is a single container with a typed **recipient list**, not a per-mode envelope: the symmetric-vs-hybrid distinction is derived from the list, not encoded in the header. v2 defines two native recipient types: `argon2id` (passphrase, exclusive — must appear alone) and `x25519` (public-key, mixable — multiple slots in one file are supported and the decrypt loop iterates). Mixed lists (e.g. `argon2id` plus `x25519`) reject before any KDF runs. See `ferrocrypt-lib/FORMAT.md` for the full byte-level spec.
