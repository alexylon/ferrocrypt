...
17. confirm the final name still denotes the object staged in step 10 (`archive/platform.rs::ObjectId`, recorded from the handle the staged root was created with and unchanged by the rename), and that the ambient `output_dir` still denotes the anchor the output was committed through; either mismatch fails the run rather than reporting an entry it did not write. The comparison follows the mode application, so a substitution made at any point up to it is caught. Both checks are inert where `std` exposes no stable identity — see §7.8;
