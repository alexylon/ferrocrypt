...
| `archive/path.rs` | `validate_archive_path` (path-traversal guard, re-exported via `fuzz_exports`), the POSIX `ustar` wire-format constants (`MAGIC`, `VERSION`, `PATH_REPRESENTABLE_MAX`, `FILE_SIZE_REPRESENTABLE_MAX`, the accepted `TYPEFLAG_REGULAR_*` / `TYPEFLAG_DIRECTORY` set, and the explicitly-forbidden `TYPEFLAG_PAX_*` / `TYPEFLAG_GNU_*` / `TYPEFLAG_SOLARIS_EXTENDED` typeflag set), and the `UstarEntryKind` classification shared by writer and reader. |
| `archive/encode.rs` | TAR archive construction (streaming, preserves directory structure). Owns `validate_encrypt_input`, the per-entry rule set (reject symlinks and non-regular/non-directory inputs); the encrypt entry points in `api.rs` call this up-front so Argon2id never fires for a symlink, and `archive` re-runs it as defense-in-depth. Hardlinks are archived as regular files. Owns `ArchiveCounters`, `ustar_archive_path_string`, `build_ustar_header` (rejects regular-file entries above `FILE_SIZE_REPRESENTABLE_MAX` so `tar::Header::set_size` cannot silently fall back to GNU binary-size encoding), the `*_mode` helpers, the `cfg(unix)` / `cfg(not(unix))` `open_no_follow` symlink guard, and the encrypt-side default mode constants. |
| `archive/decode.rs` | TAR extraction. Owns `unarchive`, the unified `extract_entries` (single arm across Linux / macOS / Windows, built on `super::platform`) which iterates via `archive.entries()?.raw(true)` so PAX/GNU/Solaris extension records surface as their own entries instead of being silently merged into the next entry by the `tar` crate, `validate_ustar_entry` (typeflag match rejects every PAX `'x'`/`'g'` and GNU `'L'`/`'K'`/`'S'`/`'M'`/`'D'`/`'V'`/`'N'` and Solaris `'X'` record explicitly), `NormalizedEntry`, `ExtractCounters`, `drain_and_verify_zero_padding`, `extract_and_register_root`, `incomplete_working_name`, `cleanup_incomplete_path`. Decryption writes under an `.incomplete` working name throughout and renames to the final name on success; on failure the staged tree is removed best-effort under the default [`IncompleteOutputPolicy::DeleteOnError`], or left for inspection / recovery under [`IncompleteOutputPolicy::RetainOnError`]. Cleanup tracks only roots this run created via `mkdir_strict` / `create_file_at`, so a pre-existing `.incomplete` left over from a previous failed run survives a retry that fails with `Previous .incomplete exists`. |
