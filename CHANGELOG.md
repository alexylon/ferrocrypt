...
### Changed
- **Breaking: native FerroCrypt Archive (FCA) format replaces the restricted POSIX ustar archive payload.** The inner archive layer that carries directory contents inside the encrypted payload is now a small native format with a manifest-first design (full path-grammar, tree-shape, and duplicate validation completes before any file is written to disk). The change bumps the outer `.fcr` wire `version` to `0x02`; old `.fcr` files become unreadable. User-visible consequences:
