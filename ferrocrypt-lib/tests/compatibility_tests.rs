...
//! produced by the v3.0 / key-v2 format. The fixtures are read-only artifacts
//! committed once and never regenerated — they represent the format as-shipped.
//!
//! ## Fixture discipline
//!
...
//! - The `generate_v3_fixtures` test is `#[ignore]` and must only be run
...
const KEY_PASSWORD: &str = "fixture-key-v3";
...
#[test]
fn test_v3_hybrid_fixture_decrypts() -> Result<(), CryptoError> {
    let decrypt_dir = tempfile::TempDir::new().unwrap();
    let passphrase = SecretString::from(KEY_PASSWORD.to_string());
...
#[test]
fn test_v3_private_key_fixture_validates() -> Result<(), CryptoError> {
    let private_key = Path::new(FIXTURE_DIR).join("v3/hybrid/private.key");
...
#[test]
fn test_v3_hybrid_binary_fixture_decrypts() -> Result<(), CryptoError> {
    let decrypt_dir = tempfile::TempDir::new().unwrap();
    let passphrase = SecretString::from(KEY_PASSWORD.to_string());
...
#[test]
fn test_v3_hybrid_directory_fixture_decrypts() -> Result<(), CryptoError> {
    let decrypt_dir = tempfile::TempDir::new().unwrap();
    let passphrase = SecretString::from(KEY_PASSWORD.to_string());
...
#[test]
fn test_v3_public_key_fingerprint_stable() -> Result<(), CryptoError> {
...
fn generate_v3_fixtures() -> Result<(), CryptoError> {
    ...
    let key_pass = SecretString::from(KEY_PASSWORD.to_string());
    ...
    println!("Fixtures generated at {}", base.display());
    Ok(())
}
