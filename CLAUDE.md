...
| `archive/model.rs` | FCA model types: `FcaHeader` (parsed header summary), `ArchiveEntryKind` (File / Directory enum), `ArchiveEntry` (path_utf8, mode, size, plus writer-only `source_path: Option<PathBuf>`), `Manifest` (entries + total_file_bytes + root_name + root_is_file). |
