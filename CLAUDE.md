...
| `protocol.rs` | Single source of truth for encrypt / decrypt orchestration plus key-pair generation (writes `private.key` + `public.key` files). Defines `pub(crate) trait RecipientScheme` (encrypt-side) and `pub(crate) trait IdentityScheme` (decrypt-side); the timing-safe slot loop covers both single-candidate (passphrase) and multi-candidate (X25519) cases. Recipient schemes return `RecipientBody`; protocol owns the header MAC verification (final acceptance gate per FORMAT.md §3.7). |
