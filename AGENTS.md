...
| `archive/path.rs` | FCA path grammar — the **single shared** writer/reader validator. `validate_fca_path` enforces UTF-8, relative-only, no leading/trailing/repeated `/`, no NUL, no backslash, byte-length cap, depth cap, no `RootDir` / `Prefix` / `CurDir` / `ParentDir` host components. `validate_fca_component` (private) enforces per-component rules: no control bytes, no Windows-reserved chars (`<>:"|?*`), no trailing dot/space, no Windows-reserved device names (`CON`, `PRN`, `AUX`, `NUL`, `CLOCK$`, `COM1..9`, `LPT1..9`) — including reserved stems with extensions (`CON.txt`, `LPT9.bin`), ASCII-case-insensitive only. `ascii_case_collision_key` lowercases ASCII A–Z for tree-side duplicate detection (used by `tree.rs`). |
