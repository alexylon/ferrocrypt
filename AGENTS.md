...
| `archive/encode.rs` | FCA writer: source-tree traversal (metadata pass) + content-streaming pass. Owns `validate_encrypt_input`, the per-entry rule set (reject symlinks, FIFOs, sockets, devices, Windows reparse points), `metadata_perm_mode` (Unix), the `cfg(unix)` / `cfg(not(unix))` `open_no_follow` symlink guard, the iterative `walk_directory` builder (heap-backed `PendingDir` stack so deeply-nested source trees cannot overflow the process stack), deterministic `sort_entries_canonically` (depth-asc + path bytes per spec §9.8), and the content-pass `stream_source_file` (reopen no-follow, fresh metadata, size-match check, `copy_exact_n` of declared size only). Hardlinks are archived as regular files. Non-Unix platforms use `DEFAULT_FILE_MODE = 0o644` / `DEFAULT_DIR_MODE = 0o755`. |
