...
| `fs/atomic.rs` | Temporary-output-name staging, no-clobber finalization for encryption output and key files, Windows / other-target path-based decrypt-promotion helpers, parent-dir `sync_all` on Unix. Linux/macOS decrypt promotion is handle-relative in `archive::platform.rs`. |
