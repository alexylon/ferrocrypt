...
| `protocol.rs` | Single source of truth for encrypt / decrypt orchestration plus key-pair generation (writes `private.key` + `public.key` files). Defines `pub(crate) trait RecipientScheme` (encrypt-side) and `pub(crate) trait DecryptionCredential` (decrypt-side). `encrypt` takes a `&[R]` recipient slice (1 element for passphrase, N for X25519) plus an `ArchiveLimits` value; rejects empty lists with `EmptyRecipientList` and multi-`Exclusive` lists with `IncompatibleRecipients` before any KDF runs. The timing-safe slot loop on decrypt covers both single-candidate (passphrase) and multi-candidate (X25519) cases. Recipient schemes return `RecipientBody`; protocol owns the header MAC verification (final acceptance gate per FORMAT.md §3.7). |
