...
| Module | Role |
|---|---|
| `lib.rs` | Public re-exports + module wiring: surfaces the `Encryptor` / `Decryptor` value-type API and `generate_key_pair` / `detect_encryption_mode` / `default_encrypted_filename` / `validate_*_key_file` from `api.rs`; re-exports `PublicKey` / `PrivateKey` (plus `RecipientKey` / `IdentityKey` aliases), `ProgressEvent`, `EncryptionMode`, `MixingPolicy`, `ArchiveLimits`; keeps the deprecated free-function compatibility surface (`symmetric_encrypt` / `symmetric_decrypt` / `hybrid_encrypt` / `hybrid_decrypt` and the matching `*Config` structs) as `#[deprecated]` thin wrappers around `protocol::*` until consumers migrate. |
