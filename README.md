...
| **`private.key`** | 9-byte header (magic, version, type, algorithm, extension length), Argon2id salt, KDF parameters, AEAD nonce, extension region, passphrase-encrypted X25519 private key (the raw key is never stored unencrypted; the entire cleartext — header, salt, KDF params, nonce, extension region — is bound as AEAD associated data so tampering fails authentication) |
| **`public.key`** | UTF-8 text file containing the canonical lowercase Bech32 `fcr1…` recipient string followed by a newline. The recipient string encodes a 1-byte algorithm identifier followed by the 32-byte X25519 public key (not secret). |
...
- **Decryption failed: header tampered after unlock** — The envelope unlocked successfully, but the header HMAC does not match. Header bytes outside the envelope — such as the `stream_nonce` or extension region — have been tampered with or corrupted since writing.
...
- **Encrypted file has unexpected trailing data** — Reserved for future dedicated trailing-data diagnostics. Current releases still reject appended bytes after the authenticated payload, but they may surface through the generic payload-authentication failure path above.
