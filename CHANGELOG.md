...
### Fixed
- **Security: `rename_no_clobber` now actually refuses to overwrite on Windows.** Previously the Windows arm fell through to `std::fs::rename`, which replaces the destination if it already exists — silently clobbering the target and defeating the "fails if destination exists" contract on that platform. It now calls `MoveFileExW` with `dwFlags = 0` via direct FFI, mirroring the `renameat2(RENAME_NOREPLACE)` / `renamex_np(RENAME_EXCL)` arms. Long paths (> `MAX_PATH`) are handled by canonicalizing via `GetFullPathNameW` and prepending the `\\?\` verbatim prefix. Added a platform-independent regression test.
- **Security: `generate_key_pair` uses non-overwriting rename for both final key files.** Previously the existence pre-check plus `fs::rename` left a TOCTOU window in which a concurrent writer could be silently clobbered; both renames now go through `rename_no_clobber`, matching the encrypted-output path.
